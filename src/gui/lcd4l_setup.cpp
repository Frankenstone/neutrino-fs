...
int CLCD4lSetup::show()
{
	...
	mc = new CMenuOptionChooser(LOCALE_LCD4L_DPF_TYPE, &temp_lcd4l_dpf_type, LCD4L_DPF_TYPE_OPTIONS, LCD4L_DPF_TYPE_OPTION_COUNT, true, NULL, CRCInput::convertDigitToKey(shortcut++));
//	mc->setHint("", LOCALE_MENU_HINT_LCD4L_DPF_TYPE);
	lcd4lSetup->addItem(mc);
	...
	switch (g_settings.lcd4l_dpf_type) {
		case 3: // PNG
			...
			max_brightness = 0;
			break;
#if BOXMODEL_VUSOLO4K || BOXMODEL_VUDUO4K || BOXMODEL_VUDUO4KSE || BOXMODEL_VUULTIMO4K || BOXMODEL_VUUNO4KSE || BOXMODEL_DM8000
		case 2: // VUPLUS4K
